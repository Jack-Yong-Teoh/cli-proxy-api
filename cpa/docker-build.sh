...
case "$choice" in
  1)
    ...
    compose up -d --remove-orphans --no-build cpa
    if [[ "${WITH_USAGE}" == "true" ]]; then
      wait_for_service
      import_stats
    fi
    echo "Services are starting from remote image."
    echo "Run 'docker compose -f ${COMPOSE_FILE} logs -f cpa' to see the logs."
    ;;
  2)
    ...
    export CPA_IMAGE="cli-proxy-api:local"
    ...
    compose up -d --remove-orphans --pull never cpa
